...
def colleagues_of_garfunkel
  ...
  execute(<<-SQL)
    SELECT coll_actors.name
    FROM movies
    JOIN castings AS garf_castings ON movies.id = garf_castings.movie_id
    JOIN actors AS garf_actors ON garf_castings.actor_id = garf_actors.id  
    JOIN castings AS coll_castings ON movies.id = coll_castings.movie_id
    JOIN actors AS coll_actors ON coll_castings.actor_id = coll_actors.id  
    WHERE garf_actors.name = 'Art Garfunkel' AND coll_actors.name != 'Art Garfunkel'
  SQL
end
